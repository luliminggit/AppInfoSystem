$("#queryCategoryLevel1").change(function () {
    ...
    if (queryCategoryLevel1 != '' && queryCategoryLevel1 != null) {
        $.ajax({
            ...
            error: function (data) {//当访问时modifyAppInfo候，404，500 等非200的错误状态码
                alert("加载二级分类失败！");
            }
        });
    } else {
...
$(".modifyVersion").on("click", function () {
    ...
    if (status == "1" || status == "3") {//待审核、审核未通过状态下才可以进行修改操作
        if (versionid == null || versionid == "") {
            ...
            window.location.href = "appversionmodify?versionId=" + versionid + "&id=" + appinfoid;
        }
    } else {
...
$(".modifyAppInfo").on("click", function () {
    ...
    var status = obj.attr("status");
    if (status == "1" || status == "3") {//待审核、审核未通过状态下才可以进行修改操作
        window.location.href = "getOneAppInfo?id=" + obj.attr("appinfoid");
    } else {
